...
function loadTodayQuestion() {
    ...
    const basicQuestionsArr = defaultQuestions;
    const customQuestionsArr = JSON.parse(localStorage.getItem('ma_custom_questions')) || [];

    const mbtiSet = pickRandom(mbtiQuestionsArr, settings.mbtiCount || 0);
    const basicSet = pickRandom(basicQuestionsArr, settings.basicCount || 0);
    const customSet = pickRandom(customQuestionsArr, settings.customCount || 0);

    const todayQuestions = [...mbtiSet, ...basicSet, ...customSet]
        .sort(() => Math.random() - 0.5); // 섞기

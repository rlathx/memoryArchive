...
// 오늘의 질문 로드 (설정된 개수대로 조합 - 분석 질문 제거 버전)
function loadTodayQuestion() {
    ...
    // 2. 설정된 개수 가져오기 (기본값 0)
    const mbtiCount = parseInt(settings.mbtiCount || 0);
    const basicCount = parseInt(settings.basicCount || 0);
    const customCount = parseInt(settings.customCount || 0);

    // 3. 랜덤으로 뽑기
    const mbtiSet = pickRandom(mbtiQuestionsArr, mbtiCount);
    const basicSet = pickRandom(basicQuestionsArr, basicCount);
    const customSet = pickRandom(customQuestionsArr, customCount);

    // 4. 질문 합치기 (MBTI -> 기본 -> 사용자 순서)
    // 순서를 섞고 싶다면 .sort(() => Math.random() - 0.5) 를 뒤에 붙이세요.
    let finalQuestions = [...mbtiSet, ...basicSet, ...customSet];

    // [삭제됨] 2주 분석 질문(twoWeekQuestion) 추가 로직을 제거하여 
    // 설정한 개수(5개)가 정확히 유지되도록 함.

    // 만약 질문이 하나도 없다면(개수 설정이 다 0일 때) 기본 질문 1개 표시
    if (finalQuestions.length === 0) {
        finalQuestions.push(defaultQuestions[0]);
    }
    ...
    listEl.innerHTML = finalQuestions.map(q => {
        let category = 'basic'; // 기본 색상
        let label = '기본 질문';

        // 질문 카테고리에 따른 색상 클래스 지정
